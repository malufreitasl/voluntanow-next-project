        ...
        {topInstitutions.map((e, index) =>
          ...
          >
            <div className=" flex flex-col bg-blue-primary  w-56 h-64 rounded-lg p-4 pl-5 gap-5">
              <div className="text-white-background  text-2xl whitespace-normal">{e.name}</div>
              <div className="bg-gray-text text-white rounded-lg text-center text-xs">{e.local}</div>
              <div className="text-white text-base whitespace-normal">{e.description}</div>
            </div>
          </motion.div>
        )}
      </div>
